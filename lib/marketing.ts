export type Post = { id:string; title:string; platform:string; format:string; owners:string[]; date:string; status:string; url:string; views:number|null; likes:number|null; comments:number|null; shares:number|null };
...
export function seedPosts():Post[] {
 ...
 const out:Post[]=[];
 const add=(title:string,owner:string,day:number,platform:string,format:string)=>out.push({id:'plan-'+out.length,title,platform,format,owners:owner==='Team'?[...staff]:owner.split(','),date:'2026-09-'+(21+day),status:'Planned',url:'',views:null,likes:null,comments:null,shares:null});
...
export function validatePost(raw:unknown):Post {
 ...
 if(!['Facebook','TikTok'].includes(p.platform)||!['Main output','Reel','Static'].includes(p.format)||!statuses.includes(p.status))throw Error('Choose a valid platform, format and status.');
